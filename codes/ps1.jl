...
function transition_values(x,p::Par)
    ...
    g_t = α*β*z*x^α  
    y_t = z*x^α
    c_t = z*x^α*(1-α*β) # y_t - g_t
    r_t = α*z/(x^(1-α))
    w_t = (1-α)*z*x^α
    return [g_t, y_t, c_t, r_t, w_t]
end
# Then what we need is to calculate at every k after a) 80% of steady state, b) productivity increases 5% permanently

# a) Capital (k) decreases to 80% of the steady state 
